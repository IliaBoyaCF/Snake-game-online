...
internal abstract class ConnectedNode : Node
{
    ...
    private IGameState ParseGameState(GameMessage.Types.StateMsg state)
    {
        ...
        return new Snake_game_online.model.Game.GameState.Game.GameState(state.State.StateOrder, fieldState, playerStates);
    }
    ...
    public static List<IPlayerState> ParsePlayerStates(GamePlayers players)
    {
        ...
        {
            playerStates.Add(new Snake_game_online.model.Game.GameState.Game.GameState.PlayerState(player.Id, player.Name, player.Score));
        }
        return playerStates;
    }
    ...
    private List<IFoodState> ParseFoodStates(RepeatedField<GameState.Types.Coord> foods)
    {
        List<IFoodState> foodStates = [];
        foreach (GameState.Types.Coord food in foods)
        {
    ...
    private List<ISnakeState> ParseSnakeStates(RepeatedField<GameState.Types.Snake> snakes)
    {
        List<ISnakeState> snakeStates = [];
        foreach (GameState.Types.Snake snake in snakes)
        {
    ...
    private ISnakeState ParseSnakeState(GameState.Types.Snake snake)
    {
        List<ILocatable.ICoordinates> body = [];
        foreach (GameState.Types.Coord bodyCoords in snake.Points)
        {
        ...
        return new Snake.SnakeState(snake.PlayerId, body, direction, snakeStatus);
    }
    ...
    private ISnakeState.SnakeStatus GetSnakeStatus(GameState.Types.Snake.Types.SnakeState state)
    {
        return state switch
        {
            GameState.Types.Snake.Types.SnakeState.Alive => ISnakeState.SnakeStatus.ALIVE,
            GameState.Types.Snake.Types.SnakeState.Zombie => ISnakeState.SnakeStatus.ZOMBIE,
